...
int PortConn_MethodCmd(ClientData clientData, Tcl_Interp* interp, int argc, const char* argv[]) {
    ...
    if (method == "get_port") {
        ...
        Tcl_CreateCommand(interp, net.c_str(), Port_MethodCmd, (ClientData)&connections[net], nullptr);

        Tcl_SetObjResult(interp, Tcl_NewStringObj(string(connection->portConn->port.name).c_str(), connection->portConn->port.name.size()));
        return TCL_OK;
        ...
        string net; // can either be constant, signal name, or net instance handle
        connection->getNet(&net);

        if (connection->netIsVar) {
            Tcl_CreateCommand(interp, net.c_str(), Port_MethodCmd, (ClientData)&connections[net], nullptr);
        }
...
int Driver_MethodCmd(ClientData clientData, Tcl_Interp* interp, int argc, const char* argv[]) {
    ...
    if (method == "name") {
        ...
        Tcl_SetObjResult(interp, Tcl_NewStringObj(string(driver->).c_str(), connection->portConn->port.name.size()));
        return TCL_OK;
    }
